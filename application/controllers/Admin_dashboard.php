...
class Admin_dashboard extends CI_Controller {
    ...
    public function index() {
        ...
        $total_stokmin = $CI->Reports->total_stokmin();
...
public function item_report() {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Admin_dashboard/item_report/');
        $config["total_rows"] = $this->Reports->item_report_count();
        $config["per_page"] = 50;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5;
        /* This Application Must Be Used With BootStrap 3 * */
        ...
        $content = $CI->lreport->item_report($links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
 // ========================= User Sales Report ==================
...
//    ========= its for filter_sales_report_category_wise ==============
    ...
    public function retrieve_dateWise_SalesReports() {
        ...
        if(!empty($alldata)){
      $perpagdata  = $this->Reports->count_retrieve_dateWise_SalesReports($from_date, $to_date,$sales);
        }else{
          $perpagdata = 50;  
        }
        $config["base_url"] = base_url('Admin_dashboard/retrieve_dateWise_SalesReports/');
        $config["total_rows"] = $this->Reports->count_retrieve_dateWise_SalesReports($from_date, $to_date,$sales);
        ...
        $content = $CI->lreport->retrieve_dateWise_SalesReports($from_date, $to_date,$sales,$links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
