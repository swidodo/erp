...
class Cinvoice extends CI_Controller {
    ...
    public function CheckInvoiceApproval(){
        // GET data
        $this->load->model('Invoices');
        $postData = $this->input->post();
        $data = $this->Invoices->getInvoiceApproveList($postData);
        echo json_encode($data);
...
// invoice list pdf download
    ...
    public function invoice_inserted_data($invoice_id) {
        ...
        // $this->template->full_admin_html_view($content);
        $invoice = $this->Invoices->retrieve_invoice_html_data($invoice_id);

        redirect('cinvoice/cetaktoko/'.$invoice_detail);
    }

    public function cetaktoko($invoice_id="",$target=null){
        if(empty($invoice_id)||!$invoice_id){
            redirect('cinvoice/manage_invoice');
        }
        $this->load->helper('aplikasi');
        $this->load->model(['Invoices','Web_settings']);
        $invoice = $this->Invoices->retrieve_invoice_html_data($invoice_id);
        $currency_details = $this->Web_settings->retrieve_setting_editdata();
        $taxfield = $this->db->select('*')
        ->from('tax_settings')
        ...
        $data = ['query'=>$invoice,'currency_details'=>$currency_details,'target'=>$target,'tax_field'=>$taxfield,'bank'=>$bank];
        if($invoice[0]['invoice_type'] == "project"){
            // $this->load->view('invoice/invoice_project_cetak',$data);
            $this->print_invoice('invoice/invoice_project_cetak',$data);
        }else{
            // $this->load->view('invoice/invoice_toko_print',$data);
           $this->print_invoice('invoice/invoice_toko_print',$data);
        }   
        
    }
    function print_invoice($view,$data){
    ...
    public function invoice_inserted_data_manual() {
        ...
        redirect('cinvoice/cetaktoko/'.$invoice_id.'/'.$invoice[0]['invoice_type']);
    }
...
//Min invoice data
    ...
    public function invoice_delete($invoice_id) {
       ...
       if(!empty($product_info)){
        ...
        foreach ($product_info as $value) {
            $json_product[] = array(
                'label'=>$value['product_id'].' '.$value['product_name'].' '.$value['size'].' ('.$value['merek'].')',
    ...
    public function product_stock_check($product_id) {
        ...
        $this->auth->check_admin_auth();
        $CI->load->model('Invoices');
        $start_stock = $CI->Invoices->get_start_stock($product_id);
        ...
        $final_total = ($start_stock + $total_purchase) - $total_sales;
        return $final_total;
    }
  ...
  $createdate=date('Y-m-d H:i:s');
    ...
    $customer_coa = [
        ...
        public function manage_do() {
            ...
            $CI->load->library('linvoice');
            $CI->load->model('Invoices');
            $content = $this->linvoice->do_list();
            $this->template->full_admin_html_view($content);
        }


        public function create_do($id){
            $CI = & get_instance();
            $CI->auth->check_admin_auth();
            $CI->load->library('linvoice');
            $content = $CI->linvoice->do_add_form($id);
            $this->template->full_admin_html_view($content);
        }
        ...
        public function do_insert() {
            ...
            $CI->auth->check_admin_auth();
            $CI->load->model('Invoices');
            $id_do = $CI->Invoices->invoice_do_entry();
            $this->session->set_userdata(array('message' => display('successfully_added')));
            redirect('cinvoice/cetakdo/'.$id_do);
    
        }


        public function cetakdo($id_do="",$target=null){
            if(empty($id_do)||!$id_do){
                redirect('cinvoice/manage_do');
            }
            $this->load->helper('aplikasi');
            $this->load->model(['Invoices','Web_settings']);
            $invoice = $this->Invoices->retrieve_do_data($id_do);
            $currency_details = $this->Web_settings->retrieve_setting_editdata();
            ...
            // filename dari pdf ketika didownload
            $file_pdf = 'Delivery Order';
            // setting paper
            $paper = 'Letter';
            //orientasi paper potrait / landscape
            $orientation = 'potrait';
            
            $html =  $this->load->view('invoice/invoice_do_cetak',$data,true);	    
        ...
        public function CheckDoList(){
            // GET data
            $this->load->model('Invoices');
            $postData = $this->input->post();
            $data = $this->Invoices->getInvoiceDoList($postData);
            echo json_encode($data);
        } 



        public function do_update_form($id_do) {
            $CI = & get_instance();
            $CI->auth->check_admin_auth();
            $CI->load->library('linvoice');
            $content = $CI->linvoice->do_edit_data($id_do);
            $this->template->full_admin_html_view($content);
        }
        ...
        public function do_update() {
            ...
            $CI->auth->check_admin_auth();
            $CI->load->model('Invoices');
            $invoice_id = $CI->Invoices->update_invoice_do();
            $this->session->set_userdata(array('message' => display('successfully_updated')));
            redirect('cinvoice/manage_do');
        }

        public function do_delete($id_do) {
            $CI = & get_instance();
            $this->auth->check_admin_auth();
            $CI->load->model('Invoices');
            $result = $CI->Invoices->delete_do($id_do);
            if ($result) {
                $this->session->set_userdata(array('message' => display('successfully_delete')));
                 redirect('Cinvoice/manage_do');
            }
        ...
        public function approval_invoice(){
            $CI = & get_instance();
            $this->auth->check_admin_auth();
            $CI->load->library('linvoice');
            $CI->load->model('Invoices');
            $content = $this->linvoice->invoice_unapprove_list();
            $this->template->full_admin_html_view($content);
        }
        ...
        public function approval($id){
            if(!$id){
                redirect('Cinvoice/approval_invoice');
            } else{
                $data = ['status'=>2];
                $this->db->where('invoice_id',$id)->update('invoice',$data);

                $data_barang = ['status'=>2];
                $this->db->where('invoice_id',$id)->update('invoice_details',$data);
            }

            $this->session->set_userdata(array('message' => 'Invoice : '.$id.' berhasil disetujui.'));
        redirect(base_url('Cinvoice/approval_invoice'));
        }
        public function autocompleteservicesearch()
        {
            $CI =& get_instance();
            $this->auth->check_admin_auth();
            $CI->load->model('Invoices');
            $service_name   = $this->input->post('service_name',TRUE);
            $service_info   = $CI->Invoices->get_ListService($service_name);
