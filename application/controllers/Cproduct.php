...
class Cproduct extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(['Suppliers','Products']);
    ...
    public function insert_product() {
        ...
        for ($i = 0, $n = count($s_id); $i < $n; $i++) {
            ...
            $supp_prd = array(
                ...
                'products_model' => $this->input->post('merek',TRUE)
            );
       ...
       foreach ($taxfield as $key => $value) {
        $data[$value] = $this->input->post($value)/100;
            ...
            $data['dimension']         = $this->input->post('dimensi',TRUE);
            ...
            $data['stock_start'] = $this->input->post('stockstart',TRUE);
            $data['stokmin'] = $this->input->post('stockmin',TRUE);
            $data['max_stock'] = $this->input->post('stockmax',TRUE);
            $data['lead_time'] = $this->input->post('leadtime',TRUE);
    ...
    {   
        ...
        $CI->load->library('lproduct');
        $CI->load->model('Products');
        $content =$this->lproduct->product_history($product_id);
        $this->template->full_admin_html_view($content);
    ...
    public function CheckProducHistory(){
        // GET data
        $this->load->model('Products');
        $postData = $this->input->post();
        // var_dump($postData);exit;
        $data = $this->Products->getProductHistory($postData);
        echo json_encode($data);
    } 
    //Add Product CSV
        ...
        if (($handle = fopen($_FILES['upload_csv_file']['tmp_name'], 'r')) !== FALSE)
        {
         ...
         while($csv_line = fgetcsv($fp,1024)){
                ...
                {                  
                  ...
                  $insert_csv['product_id']     = (!empty($csv_line[1])?$csv_line[1]:null);
                  $insert_csv['product_name']   = (!empty($csv_line[2])?$csv_line[2]:null);
                  $insert_csv['product_detail'] = (!empty($csv_line[3])?$csv_line[3]:null);
                  $insert_csv['unit']           = (!empty($csv_line[4])?$csv_line[4]:null);
                  $insert_csv['size']           = (!empty($csv_line[5])?$csv_line[5]:null);
                  $insert_csv['dimension']      = (!empty($csv_line[6])?$csv_line[6]:null);
                  $insert_csv['bobot']          = (!empty($csv_line[7])?$csv_line[7]:null);
                  $insert_csv['merek']          = (!empty($csv_line[8])?$csv_line[8]:null);
                  $insert_csv['price']          = (!empty($csv_line[9])?$csv_line[9]:null);
                  $insert_csv['category_id']    = (!empty($csv_line[10])?$csv_line[10]:null);
                  //
                  $insert_csv['stokmin']        = (!empty($csv_line[11])?$csv_line[11]:null);
                  $insert_csv['max_stock']      = (!empty($csv_line[12])?$csv_line[12]:null);
                  $insert_csv['stock_start']    = (!empty($csv_line[13])?$csv_line[13]:null);
                  $insert_csv['supplier_price'] = (!empty($csv_line[14])?$csv_line[14]:null);
               }
            ...
            if ($insert_csv['stokmin']==null)
            {
                $stockmin =0;
            }
            elseif($insert_csv['max_stock']==null)
            {
                $stockmax=0;
            }
            elseif($insert_csv['stock_start']==null)
            {
                $stockstart=0;
            }else{
                $stockmin = $insert_csv['stokmin'];
                $stockmax = $insert_csv['stock_start'];
                $stockstart = $insert_csv['max_stock'];
            }
             $data = array(
                        'product_id'    => $insert_csv['product_id'],//replace $product_id from to $insert_csv['product_id']
                        'category_id'   => $category_id,
                        'product_name'  => $insert_csv['product_name'] ,
                        'product_model' => $insert_csv['product_detail'],
                        'price'         => $insert_csv['price'],
                        'unit'          => $insert_csv['unit'],
                        'merek'         => $insert_csv['merek'],
                        'size'          => $insert_csv['size'],
                        'dimension'     => $insert_csv['dimension'],
                        'bobot'         => $insert_csv['bobot'],
                        'stokmin'       => $stockmin,
                        'stock_start'   => $stockmax,
                        'max_stock'     => $stockstart,
                        'tax'           => '',
                        'product_details'=> $insert_csv['product_detail'],
                    ...
                    if (empty($result)){
                        ...
                        $product_id = $insert_csv['product_id'];//replace $product_id from to $insert_csv['product_id'];
                                ...
                                $udata = array(
                                    ...
                                    'dimension'     => $insert_csv['dimension'],
                                    'bobot'         => $insert_csv['bobot'],
                                    'stokmin'      => $insert_csv['stokmin'],
                                    'stock_start'   => $insert_csv['stock_start'],
                                    'max_stock'     => $insert_csv['max_stock'],
                                    'tax'            => '',
                                    'product_details'=> $insert_csv['product_detail'],
                                    'image'         => base_url('my-assets/image/product.png'),
                                    'status'        => 1
                     ...
                     $supp_prd = array(
                                ...
                                'products_model' => $insert_csv['product_detail'],
                            );
