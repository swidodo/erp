...
//    ========== its for  insert_quotation =============
    public function insert_quotation() {
        ...
        $num_column = count($tablecolumn)-4;
            ...
            $userID = $this->db->SELECT('id')->FROM('user_login')->WHERE('user_id',$iduser)->get()->result_array();
            if ($this->input->post('deliverytime') !="1"||$this->input->post('quotationexp')!="1")
            {
                $delTime    = $this->input->post('deliverytimesat',TRUE)."s";
                $extTime    = $this->input->post('quotationexpsat',TRUE)."s";
            }else{
                $delTime    = $this->input->post('deliverytimesat',TRUE);
                $extTime    = $this->input->post('quotationexpsat',TRUE);
            }
            $data = array(
            ...
            'item_total_amount'     => $this->input->post('sub_total',TRUE),
            ...
            'delivery_time_sat'     => $delTime,
            'quotation_exp'         => $this->input->post('quotationexp',TRUE),
            'quotation_exp_sat'     => $extTime,
            'marketing_id'          => $userID[0]['id'],
            'status'                => $status,
            'perusahaan'            => $this->input->post('perusahaan',TRUE),
            'top'                   => addslashes($this->input->post('top',false)),
...
//    ============= its for  manage quotation ============
    ...
    public function update_quotation(){
        ...
        if ($this->input->post('deliverytime') !="1"||$this->input->post('quotationexp')!="1")
            ...
            'quotation_exp_sat'     => $extTime,
            'perusahaan'            => $this->input->post('perusahaan',TRUE),
            'top'                   => html_escape($this->input->post('top',FALSE)),
            'type'                  => $this->input->post('type',TRUE),
            );
...
//service details pdf sent to mail after adding invoice
 public function service_pdf_generate($invoice_id = null) {
    ...
    public function quotation_details_data($quot_id = null) {
        ...
        $currency_details           = $this->Web_settings->retrieve_setting_editdata();
        // $data['sales']              = $this->Quotation_model->get_User_marketing($user);
        $data['currency_details']   = $this->Web_settings->retrieve_setting_editdata();
        $data['title']              = display('quotation_details');
        $data['quot_main']          = $this->Quotation_model->quot_main_edit($quot_id);
        $data['quot_product']       = $this->Quotation_model->quot_product_detail($quot_id);
        ...
        $data['discount_type']      = $currency_details[0]['discount_type'];
        $data['company_info']       = $this->Quotation_model->retrieve_company();
        $data['rev']                = $this->Quotation_model->CountRevisiquot($quot_id);
    ...
    public function print_preview_quotation($quot_id){
        $currency_details = $this->Web_settings->retrieve_setting_editdata();
        $data['currency_details']   = $this->Web_settings->retrieve_setting_editdata();
        $data['title']              = display('quotation_details');
        $data['quot_main']          = $this->Quotation_model->quot_main_edit($quot_id);
        ...
        $data['discount_type']      = $currency_details[0]['discount_type'];
        $data['company_info']       = $this->Quotation_model->retrieve_company();
        $data['tax_id']             = $this->Quotation_model->retrieve_tax_reg();
        $data['rev']                = $this->Quotation_model->CountRevisiquot($quot_id);
       $this->load->view('quotation/quotation_printpreview', $data);
    }
    public function print_quotation($quot_id){
        $currency_details = $this->Web_settings->retrieve_setting_editdata();
        $data['currency_details']   = $this->Web_settings->retrieve_setting_editdata();
        $data['title']              = display('quotation_details');
        $data['quot_main']          = $this->Quotation_model->quot_main_edit($quot_id);
        ...
        $data['tax_id']             = $this->Quotation_model->retrieve_tax_reg();
        $data['rev']                = $this->Quotation_model->CountRevisiquot($quot_id);
    //    $this->load->view('quotation/quotation_print', $data);
        $this->load->library('pdf');
        
        // filename dari pdf ketika didownload
        $file_pdf = 'Undangan Reker 2022';
        // setting paper
        $paper = 'A4';
    ...
    // quotation revisi
    public function history_quotation()
    {
        $this->load->model('Quotation_model');
        $data['quot'] = $this->Quotation_model->getDataHistorical();
        foreach($data['quot'] as &$jml):
            $count = $this->Quotation_model->CountRevisi($jml['quotation_id']);
            $jml['count'] =$count;
        endforeach;
        $content      = $this->parser->parse('quotation/quotation_revisi_list', $data, true);
        $this->template->full_admin_html_view($content);

    }
    public function revisi_quotation()
    {
        $quotID = $this->input->post('quotation_id',TRUE);
        $user = $this->session->userdata('user_id');
        $this->load->model('Quotation_model');
        $dataQuot    = $this->Quotation_model->getQuotReplace($quotID);
        $dataProd    = $this->Quotation_model->getProdReplace($quotID);
        $dataTax     = $this->Quotation_model->getTaxReplace($quotID);
        $dataService = $this->Quotation_model->getServiceReplace($quotID);
       
        /* replace Quotation */
        foreach($dataQuot as $Quot):
            $penaw  = $this->Quotation_model->ReplaceQuot($Quot,$user);
        endforeach;

        /* replace product */
       $checkhis = $this->db->SELECT("count_revisi")
                            ->FROM ('history_quot_products_used') 
                            ->WHERE ('quot_id',$dataProd[0]['quot_id'])
                            ->ORDER_BY('count_revisi','DESC')
                            ->limit('1')
                            ->get()
                            ->result_array();
        foreach($dataProd as $Prod):
            $prods  = $this->Quotation_model->ReplaceProd($Prod,$user, $checkhis);
        endforeach;
        ...
        //                             FROM history_quotation_taxinfo 
        //                             WHERE relation_id IN('$item','$item') 
        //                             ORDER BY count_revisi DESC LIMIT 1");
        // $cktax  = $query->result_array();
        // foreach($dataTax as $Tax):
        //     $taxx = $this->Quotation_model->ReplaceTax($Tax,$user, $cktax,$data);
        // endforeach;

        /* replace service*/
        $checkserv = $this->db->SELECT("count_revisi")
                            ->FROM ('history_quotation_service_used') 
                            ->WHERE ('quot_id',$dataService[0]['quot_id'])
                            ->ORDER_BY('count_revisi','DESC')
                            ->limit('1')
                            ->get()
                            ->result_array();
        foreach($dataService as $Service):
            $serv = $this->Quotation_model->ReplaceService($Service,$user,$checkserv);
        endforeach;
        
        // if ($penaw == TRUE & $prods == TRUE & $taxx == TRUE & $serv == TRUE):
        $quot_id = $this->input->post('quotation_id',TRUE);
        $tablecolumn = $this->db->list_fields('quotation_taxinfo');
        $num_column = count($tablecolumn)-4;
        $customershow = 0;
        ...
        'quotation_exp_sat'     => $extTime,
        'perusahaan'            => $this->input->post('perusahaan',TRUE),
        'top'                   => html_escape($this->input->post('top',TRUE)),
        'type'                  => $this->input->post('type',TRUE),
        );
        ...
        $this->session->set_userdata(array('message' => 'Quotation Success Revisi'));
        redirect(base_url('Cquotation/manage_quotation')); 
            } else {
                $this->session->set_userdata(array('error_message' => display('please_try_again')));
                redirect(base_url('Cquotation'));
    ...
    public function view_revisi($quotID)
    {
        $this->load->model('Quotation_model');
        $data['data'] = $this->Quotation_model->ListRevisi($quotID);
        $content      = $this->parser->parse('quotation/quotation_detail_revisi', $data, true);
        $this->template->full_admin_html_view($content);
    }
    public function add_revisi_quotation($quot_id = null) {
        $this->load->model('Hrm_model');
      $taxfield = $this->db->select('tax_name,default_value')
              ->from('tax_settings')
              ->get()
      ...
      $data['discount_type']   = $currency_details[0]['discount_type'];
      $data['company_info'] = $this->Quotation_model->retrieve_company();
      $content = $this->parser->parse('quotation/add_quotation_revisi', $data, true);
      $this->template->full_admin_html_view($content);
  }
  public function CheckQuotList()
  {
    $this->load->model('Quotation_model');
    $postData = $this->input->post();
    $data = $this->Quotation_model->getQuotList($postData);
