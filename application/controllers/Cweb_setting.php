...
class Cweb_setting extends CI_Controller {
    ...
    public function update_setting() {
        ...
        if ($_FILES['invoice_logo2']['name']) {
            ...
            $config['encrypt_name']   = TRUE;
                ...
                $this->load->library('upload', $config);
                if (!$this->upload->do_upload('invoice_logo2')) {
                    $error = array('error' => $this->upload->display_errors());
                    $this->session->set_userdata(array('error_message' => $this->upload->display_errors()));
                    redirect(base_url('Cweb_setting'));
                } else {
                $data = $this->upload->data();  
                $invoice_logo2 = $config['upload_path'].$data['file_name']; 
                $config['image_library']  = 'gd2';
                $config['source_image']   = $invoice_logo2;
                $config['create_thumb']   = false;
                $config['maintain_ratio'] = TRUE;
                $config['width']          = 720;
                $config['height']         = 720;
                $this->load->library('image_lib', $config);
                $this->image_lib->resize();
                $invoice_logo2 = base_url() . $invoice_logo2;
        ...
        $old_invoice_logo2 = $this->input->post('old_invoice_logo2',true);
    ...
    'invoice_logo_2'    => (!empty($invoice_logo2) ? $invoice_logo2 : $old_invoice_logo2),
