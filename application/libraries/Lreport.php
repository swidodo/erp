...
public function stock_report_single_item()
    ...
    {  
        ...
        $company_info         = $CI->Reports->retrieve_company();
        $currency_details     = $CI->Web_settings->retrieve_setting_editdata();
        $category_list = $CI->Reports->stok_by_category();
        $data['currency']     = $currency_details[0]['currency'];
        $data['product_list'] = $category_list; 
        $reportList = $CI->parser->parse('report/stock_report_cetak',$data,true);
        return $reportList;
    }
...
// Retrieve todays_sales_report
    public function todays_sales_report($links = null, $per_page = null, $page = null) {
        ...
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        $company_info = $CI->Reports->retrieve_company();
        $users = $CI->db->select('users.first_name,users.last_name, sec_userrole.user_id')->join('users','users.user_id = sec_userrole.user_id')->where('sec_userrole.roleid',2)->get('sec_userrole')->result();
        $data = array(
...
// Retrieve todays_sales_report
    public function item_report($links = null, $per_page = null, $page = null) {
        $CI = & get_instance();
        $CI->load->model('Reports');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
        $sales_report = $CI->Reports->item_report($per_page, $page);
        $sales_amount = 0;
        if (!empty($sales_report)) {
            $i = 0;
            foreach ($sales_report as $k => $v) {
                $i++;
                $sales_report[$k]['sl'] = $i;
                $sales_report[$k]['sales_date'] = $CI->occational->dateConvert($sales_report[$k]['po_date']);
                $sales_amount = $sales_amount + $sales_report[$k]['total_price'];
            }
        }
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        $company_info = $CI->Reports->retrieve_company();
        ...
        $reportList = $CI->parser->parse('report/item_report', $data, true);
        return $reportList;
    }
    //======================= user sales report ================================
    ...
    public function retrieve_dateWise_SalesReports($from_date, $to_date,$sales,$links, $per_page, $page) {
        $CI = & get_instance();
        $CI->load->model('Reports');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
        $sales_report = $CI->Reports->retrieve_dateWise_SalesReports($from_date, $to_date, $sales, $per_page, $page);
        $sales_amount = 0;
        if (!empty($sales_report)) {
            $i = 0;
            foreach ($sales_report as $k => $v) {
