...
class Invoices extends CI_Model {
    ...
    //Count invoice
    public function count_invoice_do() {
        return $this->db->count_all("invoice_do");
    }
     ...
     public function getInvoiceList($postData=null){
       $this->load->library('occational');
        ...
        if($record->status == 2):
        $button .='  <a href="'.$base_url.'Cinvoice/create_do/'.$record->invoice_id.'" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('create_do').'"><i class="fa fa-file" aria-hidden="true"></i></a>';
        endif;
        if($record->quotation_id !="")
        {
           $button .='  <a href="'.$base_url.'Cinvoice_quot/detailInvoice/'.$record->invoice.'" target="_blank" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('pos_invoice').'"><i class="fa fa-print" aria-hidden="true"></i></a>';
        }else{
          $button .='  <a href="'.$base_url.'Cinvoice/cetaktoko/'.$record->invoice_id.'/listdata" target="_blank" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('pos_invoice').'"><i class="fa fa-print" aria-hidden="true"></i></a>';
        }
        ...
        if($this->permission1->method('manage_invoice','delete')->access()){
          ...
          $details ='  <a href="'.$base_url.'Cinvoice/invoice_inserted_data/'.$record->invoice_id.'" class="" >'.$record->invoice.'</a>';
            ...
            $data[] = array( 
                ...
                // 'final_date'    =>$record->date,
                'duedate'          =>$this->occational->dateConvert($record->duedate),
                'paid'    =>$record->paid,
                'paid_date'          =>$this->occational->dateConvert($record->paid_date), 
    ...
    public function getInvoiceApproveList($postData=null){
        $this->load->library('occational');
          $response = array();
          $usertype = $this->session->userdata('user_type');
          $fromdate = $this->input->post('fromdate',TRUE);
         ...
         if($record->status == 1):
         $button .='  <a href="'.$base_url.'Cinvoice/approval/'.$record->invoice_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="Persetujuan Invoice" onclick="return confirm(\'Apakah Anda yakin akan menyutujui Invoice ini?\')"><i class="fa fa-check" aria-hidden="true"></i> Disetujui</a>';
         endif;
 
       
            
           $details = $record->invoice."/".$record->perusahaan."/".romawi(intval(date('m'),strtotime($record->date))).'/'.date('Y',strtotime($record->date));
    ...
    public function getInvoiceDoList($postData=null){
        $this->load->library('occational');
          ...
          $todate   = $this->input->post('todate',TRUE);
          if(!empty($fromdate)){
             $datbetween = "(a.do_date BETWEEN '$fromdate' AND '$todate')";
          }else{
             $datbetween = "";
          }
          ## Read value
          ...
          $searchQuery = "";
          if($searchValue != ''){
             $searchQuery = " (b.customer_name like '%".$searchValue."%' or a.do_number like '%".$searchValue."%' or a.do_date like'%".$searchValue."%' or a.id_do like'%".$searchValue."%')";
          }
 
          ## Total number of records without filtering
          $this->db->select('count(*) as allcount');
          $this->db->from('invoice_do a');
          $this->db->join('invoice c','c.invoice_id = a.invoice_id');
          $this->db->join('customer_information b', 'b.customer_id = c.customer_id','left');
          ...
          ## Fetch records
          $this->db->select("a.*,b.customer_name,c.invoice,c.date,c.perusahaan");
          $this->db->from('invoice_do a');
         ...
         $button .='  <a href="'.$base_url.'Cinvoice/cetakdo/'.$record->id_do.'/listdata" class="btn btn-primary btn-sm" data-toggle="tooltip" data-placement="left" title="Cetak DO" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>';
 
       
          $button .=' <a href="'.$base_url.'Cinvoice/do_update_form/'.$record->id_do.'" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="Update DO"><i class="fa fa-pencil" aria-hidden="true"></i></a> ';
        
                                   
            $button .= '<a href="'.$base_url.'Cinvoice/do_delete/'.$record->id_do.'" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="left" title="Hapus DO"  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
             ...
             $data[] = array( 
                 ...
                 'do_number'    =>$record->do_number.'_LD/'.$record->perusahaan.'-DOS/'.romawi(intval(date('m',strtotime($record->date)))).'/'.date('Y',strtotime($record->date)),
                 'do_date'      =>$record->do_date,
                 'po_id'        =>$record->po_id,
                 'po_date'      =>$record->po_date,
                 'invoice'      =>$record->invoice.'/'.$record->perusahaan.'-INV/'.romawi(intval(date('m',strtotime($record->date)))).'/'.date('Y',strtotime($record->date)),
                 'button'       =>$button,
...
// count invoice search by customer
    ...
    public function invoice_entry() {
        ...
        // $invoice_id = $invoice_id."/CIKA-INV/".romawi(intval(date('m'))).'/'.date('Y');
    ...
    $bankcoaid='';
        ...
        if ($this->input->post('paid_amount',TRUE) >= 0) {
            ...
            $val     = $this->input->post('grand_total_price',TRUE);
            $valppn  = 0;
            $taxtot  = 0;
        ...
        //Data inserting into invoice table
         $datainv = array(
            ...
            'po_id'=>$this->input->post('pono'),
            'po_date'=>$this->input->post('tglpo'),
            ...
            'invoice_type'=>$this->input->post('tipejual')
        );
...
// Cash in Hand debit
 ...
 $bankc = array(
   ...
   $config_data = $this->db->select('*')->from('sms_settings')->get()->row();
    ...
    public function invoice_do_entry(){
        //input head
        $id_do = $this->generator(10);
        $id_do = strtoupper($id_do);
        $do_number = $this->do_number_generator();
        $data_head = [
            'id_do'       =>$id_do,
            'invoice_id'  =>$this->input->post('invoiceid'),
            'do_number'   =>$do_number,
            'do_date'     =>$this->input->post('tgldo'),
            'do_detail'   =>$this->input->post('keterangan'),
        ...
        $this->db->insert('invoice_do',$data_head);
        ...
        $remaks      = $this->input->post('remarks',TRUE);
        ...
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            ...
            $data_detail = [
                'id_do_details'=>$this->generator(15),
                'id_do'=>$id_do,
            ...
            $this->db->insert('invoice_do_details',$data_detail);
        }
    ...
    public function update_invoice_do(){
        //input head
        $id_do = $this->input->post('iddo');
        $data_head = [
            'do_date'=>$this->input->post('tgldo'),
            'type'=>$this->input->post('type'),
            'do_detail'=>$this->input->post('keterangan')
        ...
        $this->db->where('id_do',$id_do)->update('invoice_do',$data_head);
        $this->db->where('id_do',$id_do)->delete('invoice_do_details');
    ...
    public function retrieve_do_data($id_do) {
        $this->db->select('a.*,
                        b.customer_name,
                        b.customer_email,
                        b.customer_address,
        ...
        $this->db->join('invoice_do_details d', 'd.id_do = a.id_do');
        $this->db->join('product_information e', 'e.product_id = d.id_products');
    ...
    public function retrieve_invoice_editdata($invoice_id) {
        ...
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id','left');
        $this->db->join('view_invoice_details c', 'c.invoice_id = a.invoice_id','left');
        $this->db->join('product_information d', 'd.product_id = c.product_id','left');
        $this->db->join('quotation e', 'e.quotation_id = a.quotation_id','left');
        $this->db->join('bank_add f', 'f.bank_id = a.bank_id','left');
    ...
    //Retrieve invoice Edit Data
    public function retrieve_do_editdata($id_do) {
        $this->db->select("a.*,b.customer_id,b.customer_name,c.invoice,c.date, d.product_id,d.product_name, d.size, d.merek, e.unit, e.qty, e.remarks, e.desc");
        $this->db->from('invoice_do a');
        $this->db->join('invoice c','c.invoice_id = a.invoice_id');
        $this->db->join('customer_information b', 'b.customer_id = c.customer_id','left');
        $this->db->join('invoice_do_details e','e.id_do = a.id_do');
        $this->db->join('product_information d', 'd.product_id = e.id_products');
        $this->db->where('a.id_do', $id_do);
        $this->db->group_by('d.product_id');
   ...
   $bank_id = $this->input->post('bank_id',TRUE);
    ...
    $bankcoaid='';
        ...
        $data = array(
            ...
            'shipping_cost'   => $this->input->post('shipping_cost',TRUE),
            'payment_type'    =>  $this->input->post('paytype',TRUE),
            'invoice_type'    =>  $this->input->post('tipejual',TRUE),
            'po_id'    =>  $this->input->post('pono',TRUE),
            'po_date'    =>  $this->input->post('tglpo',TRUE),
...
if(!empty($this->input->post('paid_amount',TRUE))){
    ...
    public function delete_do($id_do) {
        
        $this->db->where('id_do', $id_do)->delete('invoice_do');
        //Delete invoice_details table
        $this->db->where('id_do', $id_do)->delete('invoice_do_details');
        //Delete transaction from customer_ledger table
        return true;
    ...
    public function get_start_stock($product_id){
        $query = $this->db->select('start_stock')
        ->where('product_id',$product_id)
        ->get();

        if($query->num_rows() > 0){
            $data = $query->row();
            $qty = $data->start_stock;
        } else {
            $qty = 0;
        }
...
// product information retrieve by product id
    public function get_total_product_invoic($product_id) {
        ...
        $available_quantity = ($product_information->stock_start + $total_purchase->total_purchase) - $total_sale->total_sale;
  ...
  $taxvar = [];
   for($i=0;$i<$num_column;$i++){
    ...
    //NUMBER GENERATOR
    public function do_number_generator() {
        $this->db->select_max('do_number', 'do_no');
        $query = $this->db->get('invoice_do');
        $result = $query->result_array();
...
public function customerinfo_rpt($customer_id){
    ...
    public function autocompletproductdata($product_name){
        $query=$this->db->select('*')
                ...
                ->or_like('product_id', $product_name, 'after')
    ...
    public function get_ListService($service_name)
    {
        return $this->db->select('*')
                        ->from('product_service')
                        ->like('service_name', $service_name, 'after')
                        ->order_by('service_name','asc')
                        ->get()
                        ->result_array();
    }
    public function listinvreport()
    {
    ...
    public function getPeriodeInv($start="",$end="")
    {
        if ($start !="" && $end !="")
        {
           $query = $this->db->query("SELECT a.*,b.customer_name
                                FROM invoice as a 
