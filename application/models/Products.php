...
class Products extends CI_Model {
    ...
    public function getProductList($postData=null){
      ...
      if($this->permission1->method('manage_product','update')->access()){
         ...
         $button .='  <a href="'.$base_url.'Cproduct/history/'.$record->product_id.'" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="left" title="LOG">LOG</a>';
         ...
         $supplier = '<a href="'.$base_url.'Csupplier/supplier_ledger_info/'.$record->supplier_id.'">'.$record->supplier_name.'</a>';   
            $data[] = array( 
                ...
                'description'       =>$record->product_details,
                'size'              =>$record->size,
                'dimension'         =>$record->dimension,
                'bobot'             =>$record->bobot,
                'merek'             =>$record->merek,                
                'purchase_p'        =>number_format($record->price),
                'image'             =>$image,
                'button'            =>$button,
    ...
    public function getProductHistory($postData=null){
         ...
         $columnSortOrder = $postData['order'][0]['dir']; // asc or desc
         $searchValue = $postData['search']['value']; // Search value
         $code = $postData['code']; // Search value
         // var_dump($code);exit;

         ## Search 
         $searchQuery = "";
         if($searchValue != ''){
            $searchQuery = " (a.product_id like '%".$searchValue."%' or a.type like '%".$searchValue."%' or a.product_name like '%".$searchValue."%' or a.product_details like'%".$searchValue."%' or a.po_id like'%".$searchValue."%' or a.created_at like'%".$searchValue."%') ";
         }

         ## Total number of records without filtering
         $this->db->select('count(*) as allcount');
         $this->db->from('item_history3 a');
            $this->db->where('a.product_id', $code);
    ...
    public function product_entry($data) {
            ...
            foreach ($query->result() as $row) {
                $json_product[] = array('label' => $row->product_name . "-(" . $row->merek . ")", 'value' => $row->product_id);
            }
