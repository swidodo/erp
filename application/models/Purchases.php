...
class Purchases extends CI_Model {
    ...
    public function count_final_purchases(){
       $count = $this->db->select('*')
                        ->from('view_final_pbb_buy')
                        ->get()->num_rows();
        $last_query = $this->db->last_query();
        if ($count > 0) {
            return $count;
    ...
    public function count_final_view(){
       $count = $this->db->select('*')
                        ->from('view_detail_final')
                        ->get()->num_rows();
        $last_query = $this->db->last_query();
        if ($count > 0) {
            return $count;
    ...
    public function count_barang_keluar(){
     ...
     public function getPurchaseList($postData=null){
        ...
        $button .=' <a href="'.$base_url.'Cpurchase/cetakpurchase/'.$record->purchase_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="Cetak PO" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>';
...
$this->db->join('supplier_information d', 'd.supplier_id =a.supplier_id');
        ...
        $this->db->SELECT('GROUP_CONCAT(DISTINCT p.invoice_no,"<hr>" ORDER BY p.date ASC) as inv,
                            GROUP_CONCAT(DISTINCT p.pay_duedate,"<hr>" ORDER BY p.date ASC) as due,
                            GROUP_CONCAT(DISTINCT p.payment,"<hr>" ORDER BY p.date ASC) as payment,
                            GROUP_CONCAT(DISTINCT p.tax_date,"<hr>" ORDER BY p.date ASC) as tax,
                            GROUP_CONCAT(DISTINCT p.do_number,"<hr>" ORDER BY p.date ASC) as do');
        $this->db->from('purchase_order a');
        $this->db->Join('purchase_do p','p.purchase_id=a.po_id','left');
        $this->db->where('p.purchase_id',$record->po_id);
        ...
        $do.=$dtl[0]['do'];           
        $inv.=$dtl[0]['inv'];           
        if($dtl[0]['due'] !=null): $due.= $dtl[0]['due']; else: $due.= '0000-00-00';endif;           
        if($dtl[0]['tax'] !=null): $tax.= $dtl[0]['tax']; else: $tax.= '0000-00-00';endif;          
        $button = '';
        $base_url = base_url();
        $jsaction = "return confirm('Are You Sure ?')";

        $button .=' <a href="'.$base_url.'Cpurchase/cetakpo/'.$record->po_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="Cetak PO" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>';
...
{
    // var_dump($purchaseID);exit;
        $this->db->select('c.product_id,c.qty,c.rate,c.total_price,b.supplier_name,d.product_name,d.product_details,a.po_date,e.unit,e.product_details');
        $this->db->from('purchase_order a');
        $this->db->join('supplier_information b', 'b.supplier_id = a.supplier_id','left');
        $this->db->join('purchase_order_details c', 'c.po_id = a.po_id');
        $this->db->join('product_information d','d.product_id=c.product_id','left');
        $this->db->join('product_information e', 'c.product_id = e.product_id','left');
        $this->db->where('a.supplier_id',$purchaseID['supplier_id']);
        $this->db->where('a.po_id',$purchaseID['po_id']);

        return $this->db->get()->result_array();
                    
}

public function getPurchaseSupplier($purchaseID)
{
    // var_dump($purchaseID);exit;
        $this->db->select('a.po_id,a.purchase_order,a.perusahaan');
        $this->db->from('purchase_order a');
        $this->db->where('a.supplier_id',$purchaseID['supplier_id']);
...
public function InsertdataDO($date,$purchaseId,$doId,$dodate,$paydate,$duedate,$do_details,$do_type,$invoice_no,$tax_date,$payment,$invoice_date,$user)
{
...
public function updateDopurchase($id,$date,$doId,$dodate,$paydate,$duedate,$do_details,$invoice_no,$tax_date,$payment,$invoice_date,$user)
{
    $data =[
        'date'          => $date,
        'do_number'     => $doId,
   ...
   $this->db->join('supplier_information d', 'd.supplier_id =a.supplier_id');
    ...
    $this->db->from('barang_masuk a');
    $this->db->join('barang_masuk_details b', 'b.bm_id = a.bm_id');
    $this->db->join('purchase_order e', 'e.po_id = a.po_id');
    $this->db->join('product_information c', 'c.product_id =b.product_id');
    $this->db->join('supplier_information d', 'd.supplier_id =a.supplier_id');
    ...
    foreach($records as $record ){
     ...
     $jsaction = "return confirm('Are You Sure ?')";
      ...
      $button .='  <a href="'.$base_url.'Cpurchase/cetakbarangmasuk/'.$record->bm_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="Cetak Surat Bukti Barang Masuk" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>';
//  if($this->permission1->method('manage_purchase','update')->access()){
...
//    if($this->permission1->method('manage_purchase','delete')->access()){
    ...
    // }
       ...
       $data[] = array( 
           'sl'               =>$sl,
           'po_id'        =>$record->purchase_order .'/'.$record->perusahaan,
           'supplier_name'    =>$record->supplier_name,
...
public function getBarangKeluarList($postData=null){
    $this->load->library('occational');
    $this->load->model('Web_settings');
    $currency_details = $this->Web_settings->retrieve_setting_editdata();
    $response = array();
    ...
    $searchQuery = "";
    if($searchValue != ''){
       $searchQuery = " (d.customer_name like '%".$searchValue."%' or a.barang_keluar like '%".$searchValue."%' or a.bk_date like'%".$searchValue."%')";
    }

    ## Total number of records without filtering
   $this->db->select('count(*) as allcount');
   $this->db->from('barang_keluar a');
    $this->db->join('barang_keluar_details b', 'b.bk_id = a.bk_id');
    $this->db->join('product_information c', 'c.product_id =b.product_id');
   ...
   $this->db->join('customer_information d', 'd.customer_id =a.customer_id');
    ...
    $this->db->from('barang_keluar a');
    $this->db->join('barang_keluar_details b', 'b.bk_id = a.bk_id','left');
    $this->db->join('product_information c', 'c.product_id =b.product_id','left');
    $this->db->join('customer_information d', 'd.customer_id =a.customer_id','left');
    ...
    foreach($records as $record ){
     ...
     $jsaction = "return confirm('Are You Sure ?')";
      ...
      $button .='  <a href="'.$base_url.'Cpurchase/cetakbarangkeluar/'.$record->bk_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="Cetak Surat Bukti Barang Keluar" target="_blank"><i class="fa fa-print" aria-hidden="true"></i></a>';
//  if($this->permission1->method('manage_purchase','update')->access()){
    $button .=' <a href="'.$base_url.'Cpurchase/barangkeluar_update_form/'.$record->bk_id.'" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="'. display('update').'"><i class="fa fa-pencil" aria-hidden="true"></i></a> ';
// }

//    if($this->permission1->method('manage_purchase','delete')->access()){
                             
      $button .= '<a href="'.$base_url.'Cpurchase/delete_barangkeluar/'.$record->bk_id.'" class="btn btn-danger btn-sm"  data-toggle="tooltip" data-placement="left" title="'.display('delete').'"  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
    // }
       ...
       $data[] = array( 
           ...
           'bk_date'          =>$this->occational->dateConvert($record->bk_date),
           'keterangan'       =>$record->bk_details,
           'button'           =>$button,
...
//purchase info by invoice id
 ...
 // Bank summary for credit
    ...
    public function retrieve_po_data($po_id) {
        $this->db->select('a.*,
                        b.*,
                        ((b.rate * b.qty)-b.val_discount) as ttl_price,
                        c.product_id,
                        c.product_name,
                        c.size,
                        c.merek,
    ...
    public function retrieve_purchase_data($purchase_id) {
        $this->db->select('a.*,
                        b.*,
                        b.total_amount as ttl_price,
                        c.product_id,
                        c.product_name,
                        c.size,
                        c.merek,
    ...
    public function retrieve_barang_keluar_data($id=null) {
        $this->db->select('a.*,
                        b.*,
                        c.product_id,
                        c.product_name,
        ...
        $this->db->where('a.bk_id',$id);
        $this->db->order_by('a.barang_keluar', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
 ...
 $bankc = array(
    ...
    public function barang_keluar_delete($purchase_id = null) {
        //Delete product_purchase table
        $this->db->where('bk_id', $purchase_id);
        $this->db->delete('barang_keluar');
        //Delete acc transaction
        $this->db->where('bk_id', $purchase_id);
        $this->db->delete('barang_keluar_details');
...
// purchase list for pdf
    ...
    public function update_barang_keluar(){
        ...
        $purchase_id = $this->input->post('bkid');
        $p_id = $this->input->post('product_id',TRUE);
        $customer_id = $this->input->post('customer_id',TRUE);
        $supinfo =$this->db->select('*')->from('customer_information')->where('customer_id',$customer_id)->get()->row();
        $sup_head = $supinfo->customer_id.'-'.$supinfo->customer_name;
        $purchase_order = $this->bk_number_generator();

        $p_id = $this->input->post('product_id');
        $quantity = $this->input->post('product_quantity');
        $remarks = $this->input->post('remarks');

        $this->db->where('bk_id',$purchase_id)->delete('barang_keluar_details');
        for ($i = 0, $n = count($p_id); $i < $n; $i++) {
            $product_quantity = $quantity[$i];
            $remarksx = $remarks[$i];
            $product_id = $p_id[$i];
            ...
            $data_detail = array(
                'bk_details_id' => $this->generator(15),
                'bk_id'        => $purchase_id,
                'product_id'         => $product_id,
                'qty'           => $product_quantity,
                'remarks'               => $remarksx
            ...
            if ($product_quantity > 0) {
                ...
                $this->db->insert('barang_keluar_details', $data_detail);
            }
        }
        ...
        $data_head = [
            ...
            'bk_details'=>$this->input->post('keterangan'),
            'updated_at'=>date('Y-m-d H:i:s')
        ];

        $this->db->where('bk_id',$purchase_id)->update('barang_keluar',$data_head);
...
public function barang_keluar_entry(){
        $bk_id = $this->generator(15);
        ...
        $id_user = $this->session->userdata('user_id');
        $sup_head = $supinfo->customer_id.'-'.$supinfo->customer_name;
        $bk_order = $this->bk_number_generator();
        ...
        $this->db->insert('barang_keluar',$data_head);
    ...
    public function number_generator_po() {
        ...
        $query = $this->db->get('purchase_order');
        $result = $query->result_array();
        $invoice_no = substr($result[0]['po_no'],2,10);
 
        function NoPO($num) {
            $num = $num + 1;
            switch (strlen($num)) {
                case 1  : $NoPO = "00" . $num; break;
                case 2  : $NoPO = "0" . $num; break;
                default : $NoPO = $num;
            }
            return $NoPO;
        }
        $invoice_nonex = $th."".NoPO($invoice_no);
        return $invoice_nonex;
    }
...
public function bk_number_generator() {
        $this->db->select_max('barang_keluar', 'bk_no');
        $query = $this->db->get('barang_keluar');
        $result = $query->result_array();
        $bk_no = $result[0]['bk_no'];
        if ($bk_no != '') {
            $bk_no = $bk_no + 1;
        } else {
            $bk_no = 1000;
...
public function get_list_products($postData=null){
    $this->load->library('occational');
    $this->load->model('Web_settings');
    $currency_details = $this->Web_settings->retrieve_setting_editdata();
    $response = array();
    ...
    $searchQuery = "";
    if($searchValue != ''){
       $searchQuery = " (product_name like '%".$searchValue."%' or product_id like '%".$searchValue."%' or merek like'%".$searchValue."%')";
    }

    ## Total number of records without filtering
   $this->db->select('count(*) as allcount');
   $this->db->from('view_final_pbb_buy');
     if(!empty($fromdate) && !empty($todate)){
        $this->db->where($datbetween);
    }
     if($searchValue != '')
   ...
   $this->db->from('view_final_pbb_buy');
    ...
    ## Fetch records
   $this->db->select('product_id, product_name,product_details,bobot,merek,size,dimension,price,total_amount');
   $this->db->from('view_final_pbb_buy');
     if(!empty($fromdate) && !empty($todate)){
        $this->db->where($datbetween);
    }
    ...
    foreach($records as $record ){
     ...
     $base_url = base_url();
     $jsaction = "return confirm('Are You Sure ?')";
     $button .='  <a href="'.$base_url.'Cpurchase/view_manage_product/'.$record->product_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('purchase_details').'"><i class="fa fa-window-restore" aria-hidden="true"></i></a>';
...
public function list_products_po_purchase($postData=null){
    $this->load->library('occational');
    $this->load->model('Web_settings');
    $currency_details = $this->Web_settings->retrieve_setting_editdata();
    $response = array();
    ...
    $searchQuery = "";
    if($searchValue != ''){
       $searchQuery = " (product_name like '%".$searchValue."%' or product_id like '%".$searchValue."%')";
    }

    ## Total number of records without filtering
   $this->db->select('count(*) as allcount');
   $this->db->from('view_detail_final');
     if(!empty($fromdate) && !empty($todate)){
        $this->db->where($datbetween);
    }
     if($searchValue != '')
   ...
   $this->db->from('view_detail_final');
    ...
    ## Fetch records
    $this->db->select('po_date,po_id,product_id,product_name,total_amount,isby');
    $this->db->from('view_detail_final');
    ...
    foreach($records as $record ){
     $button = '';
     $base_url = base_url();
    //  $button .='  <a href="'.$base_url.'Cpurchase/view_purchase_byId/'.$record->product_id.'" class="btn btn-success btn-sm" data-toggle="tooltip" data-placement="left" title="'.display('purchase_details').'"><i class="fa fa-window-restore" aria-hidden="true"></i></a>';
