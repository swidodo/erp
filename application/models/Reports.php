...
class reports extends CI_Model {
    ...
    public function getCheckList($postData=null){
         ...
         $searchQuery = "";
         if($searchValue != ''){
            $searchQuery = " (a.product_id like '%".$searchValue."%' or a.product_name like '%".$searchValue."%' or a.product_model like '%".$searchValue."%') ";
         }
        ...
        $this->db->group_by('a.product_id');
         ...
         $sl =1;
            ...
            $this->db->distinct();
            $this->db->select('a.invoice,a.quotation_id');
            $this->db->from('invoice a');
            $this->db->join('invoice_details b','a.invoice_id=b.invoice_id','left');
            $this->db->where('a.status',2);
            $this->db->group_by('a.quotation_id');
            $query = $this->db->get()->result_array();
            $arr = array();
            $type = array(null,'');
            foreach($query as $qw){
                array_push($arr,$qw['invoice']);
            }
            // print_r($arr);
         foreach($records as $record){
            $stockin = $this->db->select('b.quotation_id, sum(a.quantity) as totalSalesQnty')->from('invoice_details a')->join('invoice b','b.invoice_id=a.invoice_id','left')->where('a.product_id',$record->product_id)->where('a.status',2)->where_in('a.type','INV')->where_in('b.invoice',$arr)->get()->row();

            $return = $this->db->select('sum(a.quantity) as totalReturn')->from('invoice_details a')->join('invoice b','b.invoice_id=a.invoice_id','left')->where('a.product_id',$record->product_id)->where('a.status',2)->where('a.type','R')->get()->row();
        
            $prodout = $this->db->select('sum(a.qty) as prodout')->from('barang_keluar_details a')->join('barang_keluar b','a.bk_id=b.bk_id','left')->where('a.product_id',$record->product_id)->get()->row();
            ...
            $stock =  (!empty($stockout->totalPurchaseQnty)?$stockout->totalPurchaseQnty + $record->stock_start:$record->stock_start)-((!empty($stockin->totalSalesQnty)?$stockin->totalSalesQnty:0)+(!empty($return->totalReturn)?$return->totalReturn:0)+(!empty($prodout->prodout)?$prodout->prodout:0));
            $data[] = array( 
                'sl'                => $sl,                
                ...
                'totalPurchaseQnty' => sprintf('%0.2f',$stockout->totalPurchaseQnty + $record->stock_start),
                'totalSalesQnty'    => sprintf('%0.2f',$stockin->totalSalesQnty + $prodout->prodout + $return->totalReturn),
...
// supplier wise stock list
    ...
    //Retrieve todays_sales_report
    public function item_report($per_page, $page) {
        $today = date('Y-m-d');
        $this->db->select('a.*,
        b.*,
        c.product_id,
...
$this->db->select("sum(a.total_amount) as amount,count(a.invoice_id) as toal_invoice,a.*,b.first_name,b.last_name");
    ...
    //Retrieve todays_sales_report_count
    public function item_report_count() {
        $today = date('Y-m-d');
        $this->db->select("a.*,b.customer_id,b.customer_name");
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
...
//    =============== its for sales_report_category_wise_count =============
    ...
    //Retrieve all Report
    public function retrieve_dateWise_SalesReports($from_date, $to_date,$sales, $per_page, $page) {
        $this->db->select("a.*,b.*, c.first_name, c.last_name");
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->join('users c', 'c.user_id = a.sales_by');
        $this->db->where('a.date >=', $from_date);
        $this->db->where('a.date <=', $to_date);
        if($sales != 'all'){
            $this->db->where('sales_by',$sales);
        }
        $this->db->order_by('a.date', 'desc');
        $this->db->limit($per_page, $page);
        $query = $this->db->get();
    ...
    public function count_retrieve_dateWise_SalesReports($from_date="", $to_date="",$sales="") {
        $this->db->select("a.*,b.*");
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->where('a.date >=', $from_date);
...
// Total Employee Salary
    ...
    public function stok_by_category(){
        ...
        if($category_id == "all"){
            ...
            $query = $this->db->select('supplier_information.*,product_information.*,supplier_product.*')
                ...
                ->join('supplier_product', 'product_information.product_id = supplier_product.product_id', 'left')
                ->join('supplier_information', 'supplier_information.supplier_id = supplier_product.supplier_id', 'left')
                ->where('product_information.category_id',$category_id)
                ->order_by('product_information.product_name', 'asc')
                ->get();
        }
    ...
    public function total_stokmin(){
        $query = $this->db->get('stokminimal');
        return $query->result();
    }
