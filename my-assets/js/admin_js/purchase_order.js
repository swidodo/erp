...
function addPurchaseOrderField1(divName){
    ...
    if (count == limits)  {
       ...
       newdiv = document.createElement("tr");
        ...
        newdiv.innerHTML ='<td class="span3 supplier"><input type="text" name="product_name" required="" class="form-control product_name productSelection" onkeypress="product_pur_or_list('+ count +');" placeholder="Nama Produk" id="product_name_'+ count +'" tabindex="'+tab1+'" > <input type="hidden" class="autocomplete_hidden_value product_id_'+ count +'" name="product_id[]" id="SchoolHiddenId"/>  <input type="hidden" class="sl" value="'+ count +'">  </td> <td class="text-right"><input type="text" name="product_quantity[]" tabindex="'+tab2+'" required  id="cartoon_'+ count +'" class="form-control text-right store_cal_' + count + '" onkeyup="calculate_store(' + count + ');" onchange="calculate_store(' + count + ');" placeholder="0.00" value="" min="0"/>  </td><td class="test"><input type="text" name="product_rate[]" onkeyup="calculate_store('+ count +');" onchange="calculate_store('+ count +');" id="product_rate_'+ count +'" class="form-control product_rate_'+ count +' text-right" placeholder="0.00" value="" min="0" tabindex="'+tab3+'"/></td><td class="test"><input type="text" name="discount[]" onkeyup="calculate_store('+ count +');" onchange="calculate_store('+ count +');" id="discount_rate_'+ count +'" class="form-control discount_rate_'+ count +' text-right" placeholder="0.00" value="" min="0" tabindex="'+tab4+'"/><input type="hidden" name="discountval[]" required="" onkeyup="calculate_store('+ count +');" onchange="calculate_store('+ count +');" id="discountval_'+ count +'" class="form-control discountval_rate_'+ count +' text-right" placeholder="0.00" value="0" min="0"/></td><td class="text-right"><input class="form-control total_price text-right total_price_'+ count +'" type="text" name="total_price[]" id="total_price_'+ count +'" value="0.00" readonly="readonly" /> </td><td><input type="text" class="form-control text-left remark_'+ count +'" name="remark[]" placeholder="remark"></td><td> <input type="hidden" id="total_discount_1" class="" /><input type="hidden" id="all_discount_1" class="total_discount" /><button style="text-align: right;" class="btn btn-danger red" type="button"  onclick="deleteRow(this)" tabindex="8"><i class="fa fa-close"></i></button></td>';
...
function calculate_store(sl) {
    ...
    var disc          = $("#discount_rate_"+sl).val();
    var valdisc       = (item_ctn_qty * vendor_rate) * disc / 100;

    if(disc > 0){
      var total_price   = (item_ctn_qty * vendor_rate) - valdisc;
      $("#total_price_"+sl).val(total_price.toFixed(2));
      $("#discountval_"+sl).val(valdisc.toFixed(2));
    }
    else{
...
var purchasedatatable = $('#POList').DataTable({ 
         // responsive: true,
       ...
       'lengthMenu':[[10, 25, 50,100,250,300, 200, total_purchase_no], [10, 25, 50,100,250,300, 200, "All"]],
     ...
     data.csrf_test_name = csrf_test_name;
      ...
      'columns': [
         ...
         { data: 'due'},
         { data: 'tax'},
         { data: 'total_amount',class:"total_sale text-right",render: $.fn.dataTable.render.number( ',', '.', 2 )},
         { data: 'button'},
      ],
